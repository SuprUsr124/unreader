...
async function initDatabase() {
  await db.query(`
    ...
    CREATE TABLE IF NOT EXISTS messages (
      id SERIAL PRIMARY KEY, username TEXT NOT NULL, timestamp BIGINT NOT NULL, content TEXT NOT NULL, is_deleted BOOLEAN DEFAULT false
    );
    CREATE TABLE IF NOT EXISTS dms (
      id SERIAL PRIMARY KEY, sender TEXT NOT NULL, receiver TEXT NOT NULL, timestamp BIGINT NOT NULL, content TEXT NOT NULL, is_deleted BOOLEAN DEFAULT false
    );
    ...
    CREATE TABLE IF NOT EXISTS topics (
      id SERIAL PRIMARY KEY, slug TEXT UNIQUE NOT NULL, title TEXT NOT NULL, username TEXT NOT NULL, timestamp BIGINT NOT NULL
    );
    CREATE TABLE IF NOT EXISTS topic_messages (
      id SERIAL PRIMARY KEY, topic_slug TEXT NOT NULL, username TEXT NOT NULL, timestamp BIGINT NOT NULL, content TEXT NOT NULL, is_deleted BOOLEAN DEFAULT false
    );
    CREATE TABLE IF NOT EXISTS neighborhood_posts (
      id SERIAL PRIMARY KEY, username TEXT NOT NULL, title TEXT NOT NULL, content TEXT NOT NULL, timestamp BIGINT NOT NULL, is_deleted BOOLEAN DEFAULT false
    );
    CREATE TABLE IF NOT EXISTS neighborhood_comments (
      id SERIAL PRIMARY KEY, post_id INTEGER NOT NULL, username TEXT NOT NULL, content TEXT NOT NULL, timestamp BIGINT NOT NULL, is_deleted BOOLEAN DEFAULT false
    );
  `);
  console.log("Database initialized with Nested Neighborhood Commenting architecture.");
}
...
async function broadcastTopics() {
  const result = await db.query('SELECT * FROM topics ORDER BY timestamp DESC;');
...
app.get('/history', authenticateToken, async (req, res) => {
  ...
  const r = await db.query('SELECT * FROM messages ORDER BY timestamp DESC LIMIT 10 OFFSET $1;', [off]);
  res.json(r.rows.reverse());
});

app.get('/dm-history', authenticateToken, async (req, res) => {
  const off = parseInt(req.query.index ?? '0', 10) * 10;
  const r = await db.query(`SELECT id, sender AS username, receiver, timestamp, content, is_deleted FROM dms WHERE (sender = $1 AND receiver = $2) OR (sender = $2 AND receiver = $1) ORDER BY timestamp DESC LIMIT 10 OFFSET $3;`, [req.user.username, req.query.target, off]);
  res.json(r.rows.reverse());
});

app.get('/topic-history', authenticateToken, async (req, res) => {
  const off = parseInt(req.query.index ?? '0', 10) * 10;
  const r = await db.query('SELECT * FROM topic_messages WHERE topic_slug = $1 ORDER BY timestamp DESC LIMIT 10 OFFSET $2;', [req.query.slug, off]);
  res.json(r.rows.reverse());
});
...
app.get('/neighborhood-history', authenticateToken, async (req, res) => {
  const off = parseInt(req.query.index ?? '0', 10) * 10;
  try {
    const posts = await db.query('SELECT * FROM neighborhood_posts ORDER BY timestamp DESC LIMIT 10 OFFSET $1;', [off]);
    
    // Stitch all nested comments inside individual posts arrays dynamically
    for(let i=0; i<posts.rows.length; i++) {
      const comments = await db.query('SELECT * FROM neighborhood_comments WHERE post_id = $1 ORDER BY timestamp ASC;', [posts.rows[i].id]);
  ...
  ws.on('message', async (msg) => {
    try {
      ...
      if (data.type === 'auth') {
        ...
        const topicsRes = await db.query('SELECT * FROM topics ORDER BY timestamp DESC;');
      ...
      if (data.type === 'create_topic') {
        if (!isMasterAdmin(authUser)) {
          ...
          const historicalCheck = await db.query('SELECT timestamp FROM topics WHERE username = $1 AND timestamp > $2 LIMIT 1;', [authUser, sevenDaysAgo]);
        ...
        await db.query('INSERT INTO topics (slug, title, username, timestamp) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING;', [slug, data.title, authUser, Date.now()]);
        broadcastTopics();
        return;
      }

      // STREAM INTEGRATIONS (UNRESTRICTED PACKETS FOR GENERAL SPACES)
      if (data.type === 'message') {
        const dbRes = await db.query('INSERT INTO messages (username, timestamp, content) VALUES ($1, $2, $3) RETURNING id;', [authUser, Date.now(), data.content]);
        activeClients.forEach(c => c.send(JSON.stringify({ type: 'message', id: dbRes.rows[0].id, username: authUser, timestamp: Date.now(), content: data.content })));
      }

      if (data.type === 'topic_message') {
        const dbRes = await db.query('INSERT INTO topic_messages (topic_slug, username, timestamp, content) VALUES ($1, $2, $3, $4) RETURNING id;', [data.target, authUser, Date.now(), data.content]);
        activeClients.forEach(c => c.send(JSON.stringify({ type: 'topic_message', id: dbRes.rows[0].id, username: authUser, timestamp: Date.now(), content: data.content, topic_slug: data.target })));
      }

      if (data.type === 'dm') {
        const dbRes = await db.query('INSERT INTO dms (sender, receiver, timestamp, content) VALUES ($1, $2, $3, $4) RETURNING id;', [authUser, data.target, Date.now(), data.content]);
        const dmPayload = JSON.stringify({ type: 'dm', id: dbRes.rows[0].id, username: authUser, sender: authUser, receiver: data.target, timestamp: Date.now(), content: data.content });
        if (activeClients.has(data.target)) activeClients.get(data.target).send(dmPayload);
        ws.send(dmPayload);
      }

      // NEIGHBORHOOD POSTING (STRICT DOUBLE-POST RULE APPLIED HERE ONLY)
      if (data.type === 'neighborhood_post') {
        if (!isMasterAdmin(authUser)) {
          const lastPost = await db.query('SELECT username FROM neighborhood_posts ORDER BY timestamp DESC LIMIT 1;');
        ...
        const dbRes = await db.query('INSERT INTO neighborhood_posts (username, title, content, timestamp) VALUES ($1, $2, $3, $4) RETURNING id;', [authUser, data.title, data.content, Date.now()]);
        activeClients.forEach(c => c.send(JSON.stringify({ type: 'neighborhood', id: dbRes.rows[0].id, username: authUser, timestamp: Date.now(), title: data.title, content: data.content })));
      }

      // NEIGHBORHOOD NESTED COMMENTS (UNRESTRICTED CONSECUTIVE SUBMISSIONS ALLOWED)
      if (data.type === 'neighborhood_comment') {
        await db.query('INSERT INTO neighborhood_comments (post_id, username, content, timestamp) VALUES ($1, $2, $3, $4);', [data.post_id, authUser, data.content, Date.now()]);
        activeClients.forEach(c => { if(c.readyState === WebSocket.OPEN) c.send(JSON.stringify({ type: 'comment_refresh' })); });
      }

      // MODERATION LOGIC ENGINE
      if (data.type === 'mod_delete') {
        ...
        if (owner === authUser || isMasterAdmin(authUser)) {
          ...
          activeClients.forEach(c => c.send(JSON.stringify({ type: 'msg_deleted', id: data.id })));
        }
      }
      ...
      if (data.type === 'mod_delete_comment') {
        ...
        if (ownership.rows[0].username === authUser || isMasterAdmin(authUser)) {
          ...
          activeClients.forEach(c => c.send(JSON.stringify({ type: 'comment_deleted' })));
        }
      }
      ...
      if (isMasterAdmin(authUser)) {
        ...
        if (data.type === 'mod_restore') {
          ...
          activeClients.forEach(c => c.send(JSON.stringify({ type: 'msg_restored', id: data.id })));
        }
